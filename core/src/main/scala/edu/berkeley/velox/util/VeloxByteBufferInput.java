...
public class VeloxByteBufferInput extends Input {
	...
	public VeloxByteBufferInput () {
	}

	/** Creates a new Input for reading from a byte array.
	 * @param bufferSize The size of the buffer. An exception is thrown if more bytes than this are read. */
	public VeloxByteBufferInput (int bufferSize) {
		this.capacity = bufferSize;
		niobuffer = ByteBuffer.allocateDirect(bufferSize);
		niobuffer.order(byteOrder);
	}

	public VeloxByteBufferInput (byte[] buffer) {
		setBuffer(buffer);
	}

    public VeloxByteBufferInput(ByteBuffer buffer) {
      setBuffer(buffer);
    }

	/** Creates a new Input for reading from an InputStream with a buffer size of 4096. */
	public VeloxByteBufferInput (InputStream inputStream) {
		this(4096);
	...
	public VeloxByteBufferInput (InputStream inputStream, int bufferSize) {
		this(bufferSize);
		if (inputStream == null) throw new IllegalArgumentException("inputStream cannot be null.");
		this.inputStream = inputStream;
	}
	...
	public VeloxByteBufferInput (long address, int size) {

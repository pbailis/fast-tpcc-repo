...
case class LoadExamples(examples: Array[Example]) extends Request[Boolean]
case class RunPegasosAsync(gamma: Double, numIterations: Int) extends Request[DoubleVector]
...
class PegasosLoadExamplesHandler(val w: PegasosWorker) extends MessageHandler[Boolean, LoadExamples] {
  def receive(src: NetworkDestinationHandle, msg: LoadExamples) = {
    future {
      w.loadExamples(msg.examples)
      true
    }
  }
}

class PegasosRunAsyncHandler(val w: PegasosWorker) extends MessageHandler[DoubleVector, RunPegasosAsync] {
